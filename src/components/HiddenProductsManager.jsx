...
import { Upload, EyeOff, CheckCircle2, ShieldAlert, Sparkles, ExternalLink, Download } from 'lucide-react'
...
export default function HiddenProductsManager() {
  ...
  // تحميل المنتجات المحفوظة فـ الـ LocalStorage
  useEffect(() => {
    ...
    reader.onload = (event) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          // تفكيك السطر بشكل يحترم الفواصل داخل النواصص
          ...
          if (cleanRow.length >= 2) {
            ...
            // 🔗 إصلاح الـ Link بـ # تلقائياً لتفادي 404 فـ GitHub Pages
  ...
  return (
    <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-6 text-left font-sans my-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-gray-100">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            هاد المنتجات مخفيين على الزوار ومستخرجة من CSV مع إصلاح الـ Links بـ #
          </p>
        </div>
      ...
      <div className="border-2 border-dashed border-gray-200 hover:border-emerald-500 rounded-2xl p-6 text-center transition-all bg-gray-50/50">
        ...
        <label htmlFor="csv-hidden-input" className="cursor-pointer space-y-3 block">
          ...
          <div>
            <span className="text-sm font-black text-gray-900 block">اضغط هنا لرفع ملف home-and-garden.csv</span>
            <span className="text-xs text-gray-400 font-medium">سيتم معالجة العناوين والروابط بـ # تلقائياً</span>
          </div>
        </label>
      </div>

      {/* Grid المعاينة الخاص بيك كـ Admin */}
      {hiddenProducts.length > 0 && (
        <div className="space-y-4 pt-2">
          <div className="flex items-center justify-between text-xs font-bold text-gray-500">
            <span>عدد المنتجات المرفوعة: {hiddenProducts.length}</span>
            <span className="text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full font-mono">الحالة: مخفية / Admin Only</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-1">
            {hiddenProducts.map((p, idx) => (
              <div key={idx} className="p-4 rounded-2xl border border-gray-200/80 bg-white flex items-start gap-3 shadow-2xs hover:border-emerald-400 transition-all">
                <img
                  src={p.image_link}
                  alt={p.title}
                  className="w-14 h-14 object-contain rounded-xl border border-gray-100 bg-gray-50 shrink-0"
                  onError={(e) => { e.target.src = '/images/pro.jpg' }}
                />
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    >
                      <ExternalLink className="w-3 h-3" /> رابط التجربة (#)
                    </a>
                  </div>
                </div>
              </div>
